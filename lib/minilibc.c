...
int __dprintf_chk (int d, int flags, const char *format, ...){
    ...
    va_list va;
    va_start(va, format);
    int res = vdprintf(d, format, va);
    va_end(va);
    return res;
}

int __snprintf_chk(char *string, size_t size, int flags, size_t slen, const char *format, ...){
    //TODO Should not ignore flags and slen
    va_list va;
    va_start(va, format);
    int res = vsnprintf(string, size, format,va);
    va_end(va);
    return res;
}
