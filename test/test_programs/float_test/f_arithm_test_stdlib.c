//
// Created by flo on 23.06.20.
//

#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>
#include <math.h>

void init(int number, char* name);

bool check_condition(bool cond);

void assert_equals(double expected, double actual, int *failed_tests);

/**
 * Compile with:
 * riscv64-unknown-linux-gnu-gcc -o arithm compiled_test_arithm.c rv64minilibc.c rv64minilibc.h -fPIE -static -march=rv64im -mabi=lp64 -nostdlib -lgcc
 * Reference run: qemu-riscv64 arithm
 * or ./translator -v -f arithm.
 *
 * Makefile for compile:
 * arithmetic: compiled_test_arithm.c rv64minilibc.c rv64minilibc.h
	riscv64-unknown-linux-gnu-gcc -o arithm compiled_test_arithm.c rv64minilibc.c rv64minilibc.h -fPIE -static -march=rv64im -mabi=lp64 -nostdlib -lgcc
 * @param argc
 * @param argv
 * @return
 */
int main(int argc, char **argv) {
    int num = 0;
    int failed_tests;

    {
        init(++num, "1.5+1.5");
        double i = 1.5+1.5;
        assert_equals(3, i, &failed_tests);
    }

    {
        init(++num, "1.5-1.5");
        double i = 1.5-1.5;
        assert_equals(0, i, &failed_tests);
    }

    {
        init(++num, "5/2");
        double m = 5;
        m /= 2;
        assert_equals(2.5, m, &failed_tests);
    }

    {
        init(++num, "pdm");
        double n = 256;
        double o = 128;
        n += o;
        n *= 10;
        n /= 19;
        assert_equals(202, n, &failed_tests);
    }

    {
        init(++num, "3000*7");
        double n = 3000;
        double m = 7;
        assert_equals(21000, (n * m), &failed_tests);
    }

    {
        init(++num, "3000/7");
        double n = 3000;
        double m = 7;
        assert_equals(428, (n / m), &failed_tests);
    }

    {
        //div-zero quotient should have all bits set
        init(++num, "DivZero");
        double n = 256;
        double m = 0;
        assert_equals(INFINITY, (n / m), &failed_tests);
    }

    {
        //control flow experiment
        init(++num, "SumGauss");
        const int bound = 2048;

        double sum = 0;
        for (int i = 1; i <= bound; ++i) {
            sum += i;
        }

        assert_equals(2098176, sum, &failed_tests);
    }

    {
        init(++num, "MulGauss");

        //see above, we just check the gauss formula instead of the sum
        const int bound = 2048;

        double gauss = (bound * (bound + 1)) / 2;
        assert_equals(2098176, gauss, &failed_tests);
    }

    {
        //factorial experiments
        const double len = 19;
        static uint64_t expecteds[] = {
                1, 2, 6, 24, 120, 720, 5040, 40320, 362880, 3628800, 39916800, 479001600, 6227020800, 87178291200,
                1307674368000, 20922789888000, 355687428096000, 6402373705728000, 121645100408832000
        };

        for (int n = 1; n < len; ++n) {
            init(++num, "Factorial");

            //calculate the factorial
            double fact = 1;

            for (double i = 1; i <= n; i++) {
                fact *= i;
            }

            assert_equals(expecteds[n - 1], fact, &failed_tests);
        }
    }

    exit(failed_tests);
}

void init(int number, char* name) {
    printf("Test %i\t(%s)...\t", number, name);
}

bool check_condition(bool cond) {
    if (cond) {
        printf("PASSED\n");
        return true;
    } else {
        printf("FAILED\n");
        return false;
    }
}

/**
 * This is still the assert_equals from the old arithm test.
 * This will not work for the floating point extension, as any comparison with
 * actual values that have accrued rounding errors will definitely fail.
 * Nevertheless, this version will print the floats correctly,
 * as that is not implemented in the minilib.
 * @param expected
 * @param actual
 * @param failed_tests
 */
void assert_equals(double expected, double actual, int *failed_tests) {
    //check the condition, printout expected but was when false
    if (!check_condition(expected == actual)) {
        (*failed_tests)++;
        printf("Expected: %f but was: %f\n", expected, actual);
    }
}