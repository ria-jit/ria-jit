...
\begin{frame}
	...
	\begin{tikzpicture}[node distance=1.5cm]
		...
		\draw[arrow] (terminated) -- node[anchor=south] {\scriptsize no} (terminate);
		\pause
		
		\draw[arrow] (terminated) -- node[anchor=south] {\scriptsize yes} (get-pc);
	\end{tikzpicture}
\end{frame}
		...
		\begin{columns}[onlytextwidth]
			\begin{column}{0.45\textwidth}
				\begin{itemize}
					\item \textbf{RISC--V}
					\begin{itemize}				
						\item 32 general-purpose Register %todo falsch es sind 31 und x0, aber x0 ist kein general-purpose Register
