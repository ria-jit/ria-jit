...
\begin{figure}[h]
	...
	\begin{tikzpicture}
		\begin{axis}[%
			...
			\addplot+[fill=era-native, draw=black] table [x=Benchmark, y expr={\thisrow{Est. Base Run Time} / \thisrow{native}}] \nativelocaltable;
			
			% QEMU results
			\pgfplotstableread[col sep=comma]{qemu.csv}\qemutable
			\pgfplotstablecreatecol[create col/copy column from table=\nativetable{Est. Base Run Time}]{native}\qemutable
			\addplot+[fill=era-qemu, draw=black] table [x=Benchmark, y expr={\thisrow{Est. Base Run Time} / \thisrow{native}}] \qemutable;
			
			% DBT results
			\pgfplotstableread[col sep=comma]{dbt.csv}\dbttable
			\pgfplotstablecreatecol[create col/copy column from table=\nativetable{Est. Base Run Time}]{native}\dbttable
			\addplot+[fill=era-dbt-1, draw=black] table [x=Benchmark, y expr={\thisrow{Est. Base Run Time} / \thisrow{native}}] \dbttable;

			\legend{Native, QEMU, DBT}
		\end{axis}
	\end{tikzpicture}
	\caption[SPEC CPU 2017 Results]%
	{Results of \texttt{ref}-workload runs of \textit{SPEC CPU 2017}'s \texttt{intspeed} (normalised).}
\end{figure}
% ================================
