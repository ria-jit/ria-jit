...
Some of the optimization systems also have to initialized, namely the code cache, the return address stack, the register file, and the context switcher.
...
The pattern matcher (section \ref{sec:pattern-matching}) now iterates over the parsed instructions and replaces detected patterns.
...
If the next block is known but not yet translated, the last instruction will instead signal its ability to be chained to the translator before returning to the transcode loop. %todo: name
