The following section aims to provide an in-depth overview of select components central to the translator's functionality and describe the reasoning behind them. %todo name, apostrophe

\subsection{System architecture and execution control flow}
\paragraph{Initialization}\mbox{}\\
%todo: add a few words on what has to be set up and initialized here @Johannes
...
%todo: continue, elaborate? @Johannes
...
% todo @Flo Sec. 4.3.2: Fehlt: Begruendung, warum nicht z.B. 10 Register gemapped werden und mehr "lazy" zur Verfuegung stehen. Fehlt: Alternative, dass jeder Block mit einem unterschiedlichen Register-Mapping enden koennte ("This optimization opportunity is left as future work.")
...
We have built the tools required for this effort directly into the translator's analyser function, accessed via the \texttt{-a} flag. %todo name, apostrophe
...
\begin{description}
	...
	The \texttt{brk} system call must also be entirely emulated, as it would otherwise allow the guest program to modify the endpoint of the DBT's data segment (\textit{program break}), thus potentially deallocating some of the translator's memory. %todo apostrophe
	...
	In any case, we must prevent a memory mapping into the translator's memory region. %todo apostrophe
	...
	Similarly, we fail a guest \texttt{munmap} with \texttt{EINVAL} in cases where the translator's memory would be compromised by the de-allocation. %todo apostrophe, wiederholung translator's memory @flo
\end{description}
