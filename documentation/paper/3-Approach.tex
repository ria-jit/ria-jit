...
This technique, which is also implemented in the CPU for page lookup, reduces lookup time for most recent used code blocks. %todo: grammar 
...
Repeatedly retrieving many different blocks will also decrease \textit{TLB} performance as soon as their number surpasses the \textit{TLB} size, because entries will be overwritten before they are read again. %todo: better wording ?
...
The host code then emitted for the call will push its return address pair onto this stack at every execution, without having to leave guest context. %todo: alternative for "its" ?
