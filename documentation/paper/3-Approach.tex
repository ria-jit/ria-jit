...
Also a standard C compiler like the~\textit{gcc}~\cite{gcc-web} is able to emulate floating point arithmetic using integer arithmetic, using the native support of the x86-64 SSE extension is evidently a lot faster. %todo maybe include some numbers

The main difficulty (and their resolutions) that arise by using the x86-64 SSE extension to translate the RISC-V f and d extension are listed below:
\begin{itemize}
