...
In most cases we can achieve a speed of around twice the native speed, but the tests also show areas with further optimization possibilities. %todo speed seems strange here makes it sound like we are faster than native
The quite significant performance increases in comparison to QEMU can be rooted to the specialisation in RISC--V to x86--64 translation leading to a straight forward design of the translation process and enabling specific optimisations for this guest-host pairing. %todo rooted sounds strange?
...
Future standard extensions such as the \textit{"B" Standard Extension} for bit manipulation can easily be added to \translatorname when they come available by adding their opcode parsing to the parser and writing tailored translator functions. %todo their opcode parsing?
...
Besides this one could invest into on of the aforementioned areas specifically, for example applying a more rigid memory consistency model. %todo into on?
...
Also there is still potential for improvement the \textit{SPEC CPU 2017} benchmarks demonstrate its usability in translating general purpose programs from RISC--V to x86--64. %TODO what?